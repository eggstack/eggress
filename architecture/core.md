# eggress-core -- Foundation Types, Streams, Relay Facade, Chain Execution
...
compatibility facade over `eggress-relay`, protocol sniffing/dispatch, the
direct connector with DNS-rebinding protection, and the multi-hop
`ChainExecutor` used for all upstream chains.
...
- `relay(client: BoxStream, server: BoxStream) -> RelayResult` — source-compatible
  entry point; delegates to `eggress_relay::relay_with_options` with explicit
  legacy options (64 KiB buffers, `HalfClosePolicy::DrainFor(1s)`), **not** the
  generic default (unbounded drain)
- The generic engine owns the copy loop (single-task state machine, heap
  buffers, no spawned tasks); this module owns only the legacy mapping
- Mapping: rich `ClientClosed`→`ClientClosed`, rich `ServerClosed`→
  `ServerClosed`, rich `DrainTimedOut{Client|Server}`→ the first-closed side
  (matching historical observable behavior), any rich `RelayFailure`→
  debug-log (source + direction + counts) and legacy `TerminationReason::Error`
  with the captured byte counts
...
- `TerminationReason`: ClientClosed, ServerClosed, BothClosed, Error —
  `BothClosed` retained for API compatibility only; the facade in practice
  reports the first-closed side (clean completion and drain expiry) or `Error`
- `RelayError` (root-level type) is unchanged by the extraction even though the
  new engine does not use it; removing it is a separate compatibility decision
- Full engine semantics (half-close policy, cancellation, error model):
  [relay.md](relay.md)
...
- `relay()` returns `TerminationReason` rather than erroring on normal EOF;
  directional I/O details live in the `eggress-relay` rich API and are
  collapsed to `Error` (with debug logging) at this boundary
...
- `relay()` delegates to the single-task `eggress-relay` engine (no spawned copy
  tasks); dropping the relay future drops both directions with it. Cancellation
  of the outer future therefore cancels the relay with no abort grace period.
...
- `relay()` reports the first-closed side (`ClientClosed`/`ServerClosed`) when
  one direction closes first — whether the other direction follows cleanly or
  the one-second bounded drain expires. `BothClosed` is retained on
  `TerminationReason` for API compatibility but is not produced by the facade;
  `Error` is set only when a direction returns an I/O error (details in
  `eggress-relay`, debug-logged here).
