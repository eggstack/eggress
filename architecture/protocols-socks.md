...
`parse_method_negotiation` (:119) is sync for fuzzing.
...
len+bytes), 0x04=IPv6(16B). Sync parsers `parse_connect_request` (:140) and
`parse_socks5_request` (:200) for fuzzing.
...
1. `read_method_negotiation` (:258) reads version + methods.
2. `send_method_selection` (:280): password set + client offers 0x02 -> user/pass;
   no password + 0x00 -> no-auth; else 0xFF.
3. If auth: `read_auth_request` (:312) with constant-time compare (:341).
4. `read_connect_request` (:368) reads version/cmd/rsv/atyp/addr.
5. Unsupported cmd -> `handle_socks5_handshake` sends REP=0x07 (:522-528).
6. `send_connect_reply` (:476) writes success.
...
2. **`read_connect_request` rejects non-CONNECT**: The async version
   (:server.rs:368) returns `UnsupportedCommand` for BIND/UDP_ASSOCIATE.
   But `read_socks5_request` (:server.rs:421) accepts all three -- used
