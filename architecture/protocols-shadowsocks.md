...
Chunk payload max: `0x3FFF` (16383, `MAX_CHUNK_PAYLOAD` in `aead.rs:15`, duplicated at `tcp_stream.rs:15` and as `MAX_FRAME_SIZE` in `tcp.rs:13`). Zero-length payload signals end-of-stream (`tcp_stream.rs:360`).
