...
SHA224 output is 28 bytes; each byte encodes as two lowercase hex digits (`hash.rs:8-15`). Total: 56 characters. No salt, no iterations, no key stretching. This matches the Trojan protocol specification.
...
`encode_trojan_request` (`tcp.rs:30-62`) encodes all targets -- including literal IPv4 and IPv6 -- as ATYP 0x03 (domain). IPs are formatted to their standard string representation (`ip.to_string()`). This matches pproxy 2.7.9 client behavior and avoids ATYP-related interop issues with standard Trojan servers.

### Server accept parsing

`trojan_accept` (`tcp.rs:83-216`) reads:

1. **58 bytes**: 56-byte hash + CRLF.
2. **Hash comparison**: constant-time via `subtle::ConstantTimeEq` (`tcp.rs:101-106`).
3. **CRLF check**: if hash matched but CRLF is wrong, returns `AuthFailed` (not a distinguishable error) (`tcp.rs:106-111`).
4. **CMD byte**: must be 0x01 (CONNECT); others rejected with `Protocol` error (`tcp.rs:120-125`).
5. **ATYP-dependent address parsing**:
   - 0x01 (IPv4): 4 bytes IP + 2 bytes port (`tcp.rs:136-148`)
   - 0x03 (domain): 1 byte len + domain + 2 bytes port (`tcp.rs:150-179`)
   - 0x04 (IPv6): 16 bytes IP + 2 bytes port (`tcp.rs:181-195`)
6. **Domain normalization**: numeric strings in ATYP 0x03 are parsed back to `TargetHost::Ip` via `domain.parse::<IpAddr>()` (`tcp.rs:174-177`).
7. **Trailing CRLF**: must be `\r\n`; otherwise `Protocol` error (`tcp.rs:211-213`).
...
`trojan_check_password` (`hash.rs:21-24`) uses `subtle::ConstantTimeEq` on the full 56-byte hash. `trojan_accept` performs the same comparison inline (`tcp.rs:101-106`). Neither function short-circuits on first-byte mismatch.

### Oracle resistance

A correct hash followed by a malformed CRLF produces `AuthFailed` -- identical to a wrong-password response (`tcp.rs:106-111`). This prevents an attacker from distinguishing a correct password hash from an incorrect one based on error type.
