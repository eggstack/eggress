...
# Build and smoke-test the separate top-level pproxy compatibility wheel
python3.11 -m pip wheel --no-deps --wheel-dir /tmp/eggress-pproxy-compat-wheel ./python-pproxy-compat
python3.11 -m venv /tmp/eggress-pproxy-compat-test
...
# Run Milestone B asyncio adapter tests
python3.11 -c "from eggress._asyncio_adapter import open_tcp_connection, CompatibleStreamReader, CompatibleStreamWriter; print('asyncio adapter OK')"

# Run Milestone B pproxy proxy object tests
python3.11 -c "from eggress._pproxy_proxy import ProxyDirect, ProxySimple, ProxyBackward, ProxyH2, ProxySSH, ProxyQUIC, ProxyH3, AuthTable, DIRECT; d = ProxyDirect(); assert d.direct == True; assert d != ProxyDirect(); print('proxy objects OK')"

# Run Milestone B pproxy namespace tests
python3.11 -c "import pproxy; assert pproxy.Connection is pproxy.Server; assert pproxy.Connection is pproxy.server.proxies_by_uri; assert isinstance(pproxy.DIRECT, pproxy.server.ProxyDirect); print('namespace parity OK')"
...
│   ├── eggress-pproxy-compat/ # Rust pproxy compatibility: URI translation, config migration
...
│   ├── python-pproxy-compat/ # Separate pure-Python distribution installing top-level pproxy
│   ├── compat/
...
- **Stream-native transport composition**: WS/Raw/H2 intermediate-hop handlers now consume the prior-hop stream instead of opening independent connections. `RawHopHandler` passes through the stream, `WebSocketHopHandler` performs WS handshake via `connect_over_stream()`, and `H2HopHandler` uses the prior-hop stream with TLS ALPN handled by the chain executor. All intermediate-hop chains (socks5→ws, http→ws, socks5→raw, http→raw, socks5→h2, http→h2) upgraded from `compatible_with_warning` to `drop_in`.
...
- **Python protocol/cipher/plugin objects (Phase C4)**: `eggress.protocol` provides pproxy-compatible protocol objects with `MAPPINGS` dict, `get_protos()` parser, and classes (`Socks5`, `HTTP`, `SS`, `Trojan`, etc.) carrying typed metadata. Protocol classes have functional `connect()` client methods (HTTP, Socks4, Socks5) that perform the client-side handshake and return `(reader, writer)` tuples. `eggress.cipher` provides AEAD cipher objects (`AES_256_GCM_Cipher`, `AES_192_GCM_Cipher`, `AES_128_GCM_Cipher`, `ChaCha20_IETF_POLY1305_Cipher`) with `encrypt()`, `decrypt()`, `encrypt_and_digest()`, `decrypt_and_verify()` behavioral methods using the `cryptography` library; raises `UnsupportedFeatureError` otherwise. `MAP` dict and `get_cipher()` factory. `eggress.plugin` provides a bounded, cancellation-safe plugin callback bridge (`PluginBridge`) between Rust async tasks and Python callbacks with backpressure, timeout enforcement, and reentrancy detection. See `python/eggress/protocol.py`, `python/eggress/cipher.py`, and `python/eggress/plugin.py`.
- **Python asyncio semantic compatibility (Phase C5)**: `_asyncio.py` provides `AsyncBridge` (loop-affinity enforcement, cancellation propagation, contextvars preservation, stable exception conversion), `CloseWaiter` (idempotent, multi-waiter close/shutdown coordination), and `LoopAffinityError`. `_compat.py` provides Python version helpers (`PY_VERSION`, `HAS_TASKGROUP`, `HAS_EXCEPTIONGROUP`, `CANCELLED_ERROR_BASE`). `AsyncConnection` uses `AsyncBridge` + `CloseWaiter` for idempotent close and multi-waiter `await_closed()`. `AsyncEggressHandle` uses `AsyncBridge` for all async operations. `Server.aclose()`/`wait_closed()` use `CloseWaiter` for race-safe shutdown. `PluginBridge` reentrancy detection uses `contextvars.ContextVar` (task-level isolation, not thread-level). 107 semantic tests in `test_asyncio_semantic.py` covering loop affinity (including cross-loop detection via live threads), bridge lifecycle, cancellation (cancel_wait_closed, cancel during aclose, concurrent bridge.cancel, plugin callback cancellation), close ordering, contextvars, exception chaining, asyncio debug mode (with real async operations), interpreter safety (repeated asyncio.run cycles, GC, context managers), version compat, stress/race (AsyncConnection, Server, CloseWaiter, PluginBridge), representative pproxy async patterns (server lifecycle, context manager, hot reload, plugin callbacks), and manifest/doc agreement. See `python/eggress/_asyncio.py`, `python/eggress/_compat.py`, and `python/tests/test_asyncio_semantic.py`.
- **Milestone B: Python source-compatibility preview (under corrective review)**: The `eggress-pproxy-compat` distribution exposes the public pproxy namespace required by the strict manifest. `pproxy.Connection` and `pproxy.Server` are both aliases for `pproxy.server.proxies_by_uri` (a function, matching pproxy 2.7.9). `pproxy.Rule` is an alias for `compile_rule`. `pproxy.DIRECT` is a `ProxyDirect()` instance (singleton). The proxy object hierarchy (`ProxyDirect` → `ProxySimple` → `ProxyBackward`/`ProxyH2`/`ProxySSH`/`ProxyQUIC`/`ProxyH3`) carries typed metadata. `ProxySimple` now routes through upstream proxies (its `direct` property returns `False` and carries `jump`/`protos`/`cipher` metadata). `AuthTable` shares state across instances for the same `remote_ip` (matching pproxy 2.7.9 behavior); different IPs are isolated. `tcp_connect()` is a coroutine returning asyncio-compatible `(reader, writer)` pairs via the `CompatibleStreamReader`/`CompatibleStreamWriter` adapter. Protocol classes (`HTTP`, `Socks4`, `Socks5`) have functional `connect()` client methods that perform the client-side handshake. `channel()` always relays bytes regardless of stat callback — it is a pure byte relay, not a protocol-aware handler. **Closure status:** AuthTable IP sharing, ProxySimple upstream routing, and protocol client methods are now implemented. See `plans/MILESTONES_A_C_POST_IMPLEMENTATION_CORRECTIVE_CLOSURE.md`.
...
- **Python packaging**: Canonical package `eggress` remains the Rust-backed PyPI wheel and never mutates `sys.modules` to alias `pproxy`. The separate `python-pproxy-compat/` project publishes `eggress-pproxy-compat`, installs the top-level `pproxy` namespace for the certified subset, pins the matching `eggress` version, and declares the `cryptography` dependency. Wheels are built for 5 platforms via maturin; the compatibility wheel is pure Python and must be tested in a clean environment. See `docs/adr/ADR_python_import_and_distribution_strategy.md`.
...
- **Cipher behavioral closure (Milestone C)**: AEAD cipher objects (`AES_256_GCM_Cipher`, `AES_192_GCM_Cipher`, `AES_128_GCM_Cipher`, `ChaCha20_IETF_POLY1305_Cipher`) now implement `encrypt()`, `decrypt()`, `encrypt_and_digest()`, `decrypt_and_verify()` using the `cryptography` library when available. Milestone C (C9/C10) extended functional encrypt/decrypt to stream ciphers (RC4, RC4-MD5, ChaCha20, ChaCha20-IETF, AES-CFB/CFB8/OFB/CTR 128/192/256) using the `cryptography` library; RC4 uses a pure-Python implementation. Salsa20, Blowfish, CAST5, and DES remain unsupported (no `cryptography` backend). `_evp_bytes_to_key()` corrected to match OpenSSL's EVP_BytesToKey algorithm (password included in each hash iteration). `-py` variant aliases added to the cipher MAP for pproxy 2.7.9 compatibility. `PacketCipher.encrypt()`/`decrypt()` functional for AEAD ciphers (C11). `pproxy.proto.sslwrap()` is a functional TLS wrapping function (C8).
...
- **Milestone A: Honest pproxy compatibility contract (under corrective review)**: Replaces capability-oriented claims with a frozen, mechanically validated, behavior-oriented contract against pproxy==2.7.9. Artifacts: oracle provenance (`compat/pproxy-2.7.9/`), strict manifest (`docs/parity/pproxy_2_7_9_strict_manifest.toml`) with `certification_scope` and `closure_required` fields, compatibility policy (`docs/parity/PPROXY_COMPATIBILITY_POLICY.md`), known-defects registry. Strict manifest reserves `drop_in` for behavioral matches validated by paired oracle/candidate testing. AuthTable now shares state across instances for the same `remote_ip` (matching pproxy 2.7.9); `ProxySimple` routes through upstream proxies (`direct` returns `False`, carries `jump`/`protos`/`cipher`). Protocol classes (`HTTP`, `Socks4`, `Socks5`) have functional `connect()` client methods. **Corrective closure pass:** The post-implementation corrective closure pass (`plans/MILESTONES_A_C_POST_IMPLEMENTATION_CORRECTIVE_CLOSURE.md`) fixes direct-bypass routing, `socks_address` contract mismatch, `start_server()` handler invocation, and incomplete `http_channel()` semantics. Prior completion records (`docs/PHASE_54_MILESTONES_A_C_CORRECTIVE_PASS_COMPLETION.md`) are superseded. See `docs/parity/PPROXY_A_C_SKIP_INVENTORY.toml` for the machine-readable skip inventory.
