...
- **Native OutboundConnector**: `OutboundConnector` in `eggress-embed::outbound` provides `from_toml()`, `from_pproxy_uri()`, `connect_tcp()`, `connect_tcp_timeout()`, `associate_udp()` for native Rust outbound connections without temporary local listeners. Python bindings expose `PyOutboundConnector` with `from_pproxy_uri()`, `from_toml()`, `validate_config()`, `upstream_count()`, and `preview_connect()`. `HopHandler::handshake()` now accepts `hop_index: usize` for cross-chain pool isolation.
