...
- **pproxy CLI native-equivalent closure (Phase 38)**: `--ssl`, `-b`, `--rulefile` now generate TOML config (TLS listener, reject rules, rulefile-parsed rules). `--log`, `--sys`, `--get`, `--test`, `--pac`, `--reuse`, `-a` emit structured diagnostics mapping to native eggress capabilities. `--daemon` remains unsupported. See `plans/phase_38_pproxy_cli_native_equivalent_closure.md` and `docs/cli/PPROXY_CLI_INVENTORY.md`.
