...
- **Milestone B: Python source-compatibility preview (under corrective review)**: The `eggress-pproxy-compat` distribution exposes the public pproxy namespace required by the strict manifest. `pproxy.Connection` and `pproxy.Server` are both aliases for `pproxy.server.proxies_by_uri` (a function, matching pproxy 2.7.9). `pproxy.Rule` is an alias for `compile_rule`. `pproxy.DIRECT` is a `ProxyDirect()` instance (singleton). The proxy object hierarchy (`ProxyDirect` → `ProxySimple` → `ProxyBackward`/`ProxyH2`/`ProxySSH`/`ProxyQUIC`/`ProxyH3`) carries typed metadata. `tcp_connect()` is a coroutine returning asyncio-compatible `(reader, writer)` pairs via the `CompatibleStreamReader`/`CompatibleStreamWriter` adapter. **Corrective pass note:** The initial implementation pass claimed gaps reduced from 83 to 2, but the strict report on disk still shows 83 gaps at 57% readiness. Multiple `drop_in` records use only `module_existence` as evidence. Milestones A–C are reopened. See `plans/MILESTONES_A_C_CORRECTIVE_PASS.md`.
