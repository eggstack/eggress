...
- Crates.io is fully manual (`cargo publish --dry-run`, then dependency order, facades last). `scripts/publish-remaining.sh` encodes the tier order (28 crates; `eggress-outbound` precedes `eggress-server`, with `eggress-metrics`/`eggress-admin`/`eggress-runtime` following the server). Versions immutable: roll forward, never replace.
