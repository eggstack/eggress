...
- Start subsystem work at `architecture/overview.md` (index into per-component deep dives), not by re-deriving layout from source.
- Skills live in `.skills/` (mirrored via symlinks into `.agents/skills/`, `.opencode/skills/` — add new skills in `.skills/` plus a symlink in each mirror). Load via the `skill` tool: `rust-proxy-dev`, `python-bindings`, `testing` are the most general; also `security-dev`, `config-reload`, `routing-rules`, `udp-protocol`, `advanced-transports`, `reverse-proxy`, `release`.
