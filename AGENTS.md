...
- Standalone CLI install is `docs/INSTALLATION.md` (binary installer preferred; `cargo install eggress-cli --locked` is the Rust/developer path). `eggress version` prints `eggress X.Y.Z`; there is no `eggress update` command — updates are installer re-runs.
