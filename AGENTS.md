...
- **Milestone B: Python source-compatibility preview**: The `eggress-pproxy-compat` distribution now exposes the complete public pproxy namespace required by the strict manifest. `pproxy.Connection` and `pproxy.Server` are both aliases for `pproxy.server.proxies_by_uri` (a function, matching pproxy 2.7.9). `pproxy.Rule` is an alias for `compile_rule`. `pproxy.DIRECT` is a `ProxyDirect()` instance (singleton). The proxy object hierarchy (`ProxyDirect` → `ProxySimple` → `ProxyBackward`/`ProxyH2`/`ProxySSH`/`ProxyQUIC`/`ProxyH3`) carries typed metadata with working `direct`, `bind`, `lbind`, `unix`, `alive`, `connections`, `rproto`, `auth`, `jump`, `destination`, `match_rule`, `logtext`, and `connection_change` methods. `tcp_connect()` is a coroutine returning asyncio-compatible `(reader, writer)` pairs via the `CompatibleStreamReader`/`CompatibleStreamWriter` adapter (`_asyncio_adapter.py`), which bridges Rust-owned `AsyncOutboundStream` into the standard asyncio stream interface. `udp_sendto()`, `open_connection()`, `prepare_connection()`, and `start_server()` raise `NotImplementedError` for runtime integration (deferred to later milestones). Strict manifest gaps reduced from 83 to 2. See `plans/MILESTONE_B_PYTHON_SOURCE_COMPATIBILITY.md`.
