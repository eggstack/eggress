...
- 27 crates in `crates/`: `eggress-relay` (leaf byte-relay engine) + `eggress-core` / `eggress-uri` / `eggress-config` / `eggress-routing` / `eggress-metrics` (foundation), `eggress-server` / `eggress-runtime` / `eggress-admin` / `eggress-udp` / `eggress-system-proxy`, `eggress-protocol-{http,socks,shadowsocks,trojan,websocket,raw,reverse,h3}`, `eggress-transport-{tls,ssh,quic}`, facades `eggress-cli` (installs both `eggress` and `pproxy` binaries) / `eggress-embed` / `eggress-python` / `eggress-pproxy-compat`, test support `eggress-testkit`. Root package is `eggress-bench` (Criterion in `benches/`).
