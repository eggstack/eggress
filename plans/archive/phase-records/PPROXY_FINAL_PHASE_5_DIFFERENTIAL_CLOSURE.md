...
- **Focused pproxy 2.7.9 source/oracle evidence**: DUMMY callable identity, UDP_LIMIT=30 — verified as upstream matches. Eggress intentionally raises `UnsupportedPProxyFeature` for non-`None` `prepare_ciphers` and plugin-bearing URIs because private cipher/plugin stream wrapping is outside the bounded compatibility target; these are fail-closed decisions, not upstream-equivalent behavior.
