...
- `archive/` — completed or superseded interim planning retained for traceability. The pre-transition flat phase records live under `archive/phase-records/`.
- `registry.md` — compact index of active subsystem roadmaps, implementation plans, closure work, and dependencies.

## Core rule

Long-term documents state **what Eggress is becoming and what must remain true**. Interim documents state **what an agent should implement next against a specific repository baseline**.
...
Register active work in `registry.md` before handing implementation plans to agents. An implementation handoff is active only when `docs/ROADMAP.md` explicitly registers it alongside `registry.md`.

## Authority

Current authority: `docs/ROADMAP.md` (canonical roadmap), `plans/registry.md` (active planning control surface), `docs/parity/pproxy_capability_manifest.toml` + `docs/parity/PPROXY_PRACTICAL_COMPATIBILITY_MATRIX.md` (compat contract), `docs/CI_STATUS.md` (verification policy), `docs/TESTING.md` (suite inventory). `EGGRESS_ROADMAP.md` at root is the original roadmap, retained for provenance.
