# eggress Phase 1 Development Plan
...
Build the minimum architecture that can support the entire eggress roadmap without later replacing the core connection model.

At phase completion, eggress must be a useful standalone TCP proxy supporting:
...
eggress -r socks5://user:pass@proxy.example:1080
...
`clap` should collect raw values. The eggress parser should produce the typed configuration and all validation errors before listeners are started.
...
Running `eggress` with no arguments should start a loopback-safe or reference-compatible mixed HTTP/SOCKS listener. The exact bind address must follow the frozen compatibility decision. If reference behavior binds publicly, eggress may require an explicit compatibility profile to do so; the deviation must be documented.
...
1. eggress client → Python `pproxy` server;
2. Python `pproxy` client → eggress server.
