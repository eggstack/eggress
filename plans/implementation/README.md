...
There is currently no active implementation plan. New plans are registered in `plans/registry.md` and `docs/ROADMAP.md` before handoff.
...
Milestone numbering is local to the subsystem roadmap unless the roadmap defines another stable identifier. Subsystem names: `proxy-core-relay`, `server-runtime-config`, `routing-health-observability`, `protocols-edge`, `transports`, `outbound-chains`, `parity-compat`, `delivery`.
...
Describe the relevant code, tests, config, protocols, guards, and known gaps at the repository baseline.
...
### Routing/health/observability

### Protocols/transports/chain

### Compat translation and manifest

### Delivery (CLI/embed/Python/release)
...
Describe backward compatibility, data migration, protocol negotiation, configuration fallback, tier/manifest effects, and removal criteria for legacy paths.
...
- register the plan in `plans/registry.md` and `docs/ROADMAP.md`.

The implementation agent may adjust file-level mechanics based on current code. It may not weaken canonical invariants or silently enlarge scope. A compat-claim change additionally requires the manifest plus oracle/differential/interop evidence.
