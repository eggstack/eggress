...
> **Status:** The canonical contract is the 148-capability manifest at `docs/parity/pproxy_capability_manifest.toml`. The Rust-native CLI and runtime are production-ready. The Python source-compatibility surface (`eggress-pproxy-compat`) is experimental and was reopened via the Milestones A–C corrective pass to replace structural/namespace evidence with paired oracle-backed behavioral evidence. The corrective pass is **closed against all locally-verifiable acceptance criteria** — see `docs/PHASE_54_MILESTONES_A_C_CORRECTIVE_PASS_COMPLETION.md` for the full closure record (30/31 acceptance criteria met; the single unmet criterion is hosted CI evidence retention, blocked by repository billing per `docs/CI_STATUS.md`). The strict report (`docs/parity/PPROXY_2_7_9_STRICT_REPORT.md`) is generated from the manifest and freshness-enforced. **Strict full drop-in parity with pproxy is not yet achieved** — the strict report honestly classifies unsupported transports (SSH, QUIC/HTTP/3, SSR/legacy Shadowsocks) as `gap`, and two specific capabilities (`cli.get`, `process.reload.routing`) remain gaps for milestone B closure. See `docs/PPROXY_PARITY_SPEC.md` for the tier taxonomy.
...
**Corrective pass note:** The initial implementation pass produced the infrastructure above but did not satisfy all acceptance gates. The Milestones A–C corrective pass has since replaced prior structural-only `drop_in` evidence with paired oracle-backed behavioral evidence; the strict report is now generated from the manifest and freshness-enforced. The corrective pass is closed against all locally-verifiable acceptance criteria (30/31). Two capabilities (`cli.get`, `process.reload.routing`) remain honest `gap` records. Unsupported transports (SSH, QUIC/HTTP/3, SSR/legacy Shadowsocks) remain intentional non-parity. This is **not** a claim that eggress is a complete drop-in replacement for pproxy — see `docs/PHASE_54_MILESTONES_A_C_CORRECTIVE_PASS_COMPLETION.md`.
