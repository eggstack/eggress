...
> **Status:** The canonical contract is the 148-capability manifest at `docs/parity/pproxy_capability_manifest.toml`. The Rust-native CLI and runtime are production-ready. The Python source-compatibility surface (`eggress-pproxy-compat`) is **experimental** and **REOPENED — final evidence and runtime closure in progress**. Post-completion review found additional behavioral and evidence defects after the initial corrective pass; see `plans/MILESTONES_A_C_FINAL_EVIDENCE_RUNTIME_CLOSURE.md` for the current closure pass. **Strict full drop-in parity with pproxy is not yet achieved** — the strict report honestly classifies unsupported transports (SSH, QUIC/HTTP/3, SSR/legacy Shadowsocks) as `gap`, and two specific capabilities (`cli.get`, `process.reload.routing`) remain gaps. See `docs/PPROXY_PARITY_SPEC.md` for the tier taxonomy.
