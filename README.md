...
> **Status:** The canonical contract is the 148-capability manifest at `docs/parity/pproxy_capability_manifest.toml`. The Rust-native CLI and runtime are production-ready. The Python source-compatibility surface (`eggress-pproxy-compat`) is **experimental** and **REOPENED — post-implementation corrective closure in progress**. See `plans/MILESTONES_A_C_POST_IMPLEMENTATION_CORRECTIVE_CLOSURE.md` for the current active closure pass. **Strict full drop-in parity with pproxy is not yet achieved** — the strict report honestly classifies unsupported transports (SSH, QUIC/HTTP/3, SSR/legacy Shadowsocks) as `gap`, and two specific capabilities (`cli.get`, `process.reload.routing`) remain gaps. See `docs/PPROXY_PARITY_SPEC.md` for the tier taxonomy.
...
**Corrective pass note:** The initial implementation pass produced the infrastructure above but did not satisfy all acceptance gates. The Milestones A–C corrective closure pass (see `plans/MILESTONES_A_C_POST_IMPLEMENTATION_CORRECTIVE_CLOSURE.md`) is the current active plan. **Milestones A–C are reopened** — post-completion review found additional behavioral and evidence defects including direct-bypass routing, `socks_address` contract mismatch, `start_server()` handler invocation, and incomplete `http_channel()` semantics. Prior completion records (`docs/PHASE_54_MILESTONES_A_C_CORRECTIVE_PASS_COMPLETION.md`) are superseded. Two capabilities (`cli.get`, `process.reload.routing`) remain honest `gap` records. Unsupported transports (SSH, QUIC/HTTP/3, SSR/legacy Shadowsocks) remain intentional non-parity. This is **not** a claim that eggress is a complete drop-in replacement for pproxy.
...
> `plans/MILESTONES_A_C_POST_IMPLEMENTATION_CORRECTIVE_CLOSURE.md` for the current closure plan
> and status.
