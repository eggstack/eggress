# eggproxy Full-Parity Roadmap

## Project definition

**eggproxy** is a Rust-native, embeddable, multi-protocol proxy framework and CLI intended to reach practical and behavioral parity with Python's `pproxy` while preserving a nearly identical command-line and URI-driven usage model.

The project should not be structured as a literal source rewrite. `pproxy` combines several distinct concerns—protocol termination, transport composition, proxy chaining, routing, UDP association management, encryption, transparent proxying, administration, and platform integration—behind a compact interface. eggproxy should preserve that interface while replacing the internals with explicit, typed Rust abstractions.
...
These capabilities may later integrate with eggproxy, but they should not distort the core architecture.
...
eggproxy -l ss://chacha20:password@:8388
eggproxy -r ss://hop1:8388__http://hop2:8080
eggproxy -ul socks5://:1080 -ur ss://server:8388
```

### Protocol parity

eggproxy should eventually support all major protocol and transport classes exposed by `pproxy`:
...
A protocol is not considered complete until eggproxy can interoperate in both directions with:
...
│   ├── eggproxy-protocol-shadowsocks/
│   ├── eggproxy-protocol-trojan/
│   ├── eggproxy-protocol-ssh/
│   ├── eggproxy-transport-tls/
│   ├── eggproxy-transport-websocket/
│   ├── eggproxy-transport-quic/
│   ├── eggproxy-transparent/
│   ├── eggproxy-admin/
│   └── eggproxy-testkit/
└── tests/
    ├── interoperability/
...
eggproxy -l socks4+socks5://:1080
eggproxy -l http+socks4+socks5://:8080
eggproxy -r http://proxy:8080
eggproxy -r socks5://proxy:1080
...
Make eggproxy operable as a long-running service with route policy, upstream groups, diagnostics, and administration.
...
eggproxy interoperates with Python `pproxy` and `shadowsocks-rust` for supported modes, and with an independent Trojan implementation.
...
- versioned eggproxy tunnel framing;
...
Recover original destinations from kernel redirection and route connections through eggproxy.
