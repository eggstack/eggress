...
1. [Outbound TCP Socket Metadata Recovery](../plans/OUTBOUND_TCP_SOCKET_METADATA_RECOVERY.md) — **READY FOR IMPLEMENTATION**. Add an additive metadata-preserving core/chain seam so `OutboundConnector` reports the actual local and peer socket addresses for direct and TCP-backed first-hop routes, removes the metadata-only second DNS lookup, and preserves every existing connection signature/boxed-stream boundary.
