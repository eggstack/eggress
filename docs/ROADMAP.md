...
1. [Outbound TCP Socket Metadata Recovery](../plans/OUTBOUND_TCP_SOCKET_METADATA_RECOVERY.md) — **IMPLEMENTATION COMPLETE; FULL GATES PENDING**. Add an additive metadata-preserving core/chain seam so `OutboundConnector` reports the actual local and peer socket addresses for direct and TCP-backed first-hop routes, removes the metadata-only second DNS lookup, and preserves every existing connection signature/boxed-stream boundary.
2. [Outbound TCP Socket Metadata Release Qualification](../plans/OUTBOUND_TCP_SOCKET_METADATA_RELEASE_QUALIFICATION.md) — **READY AFTER PHASE 1**. Prepare the resulting correction as the next immutable lockstep patch release (expected 1.0.9 if no intervening release), with version/package/release qualification only. Registry publication and any `v*` tag remain maintainer-operated and require separate explicit authorization.
