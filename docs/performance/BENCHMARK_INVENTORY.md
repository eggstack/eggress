...
|-----------|------|---------|----------|-------------|
| `tcp_relay` | `benches/tcp_relay.rs` | Setup-inclusive and steady-state TCP relay via `eggress-relay` (1KB, 64KB, 1MB, bounded concurrency) + `copy_bidirectional` baseline; explicit 16/32/64 KiB buffer matrix | ~90s | Tier 0 |
...
| `tls_setup` | `benches/tls_setup.rs` | Cached default client access, executor construction, raw server-config construction, and component prepared-listener/per-connection TLS handshake comparison | ~30s | Tier 0 |
