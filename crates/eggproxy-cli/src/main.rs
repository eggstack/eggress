...
use clap::Parser;
use eggproxy_core::chain::{ChainExecutor, HopHandler};
use eggproxy_core::connector::{Connector, DirectConnector};
use eggproxy_core::listener::{TcpListener, TcpListenerConfig};
use eggproxy_core::relay::relay;
use eggproxy_core::{BoxStream, TargetAddr, TargetHost};
use eggproxy_uri::{parse_proxy_chain, CredentialSpec, ProxyChainSpec};
...
/// EggProxy - A multi-protocol TCP proxy
#[derive(Parser, Debug)]
#[command(name = "eggproxy", version, about = "A multi-protocol TCP proxy")]
struct Cli {
...
async fn main() {
    ...
    let upstream_chain: Option<ProxyChainSpec> = if args.upstreams.is_empty() {
        ...
        match parse_proxy_chain(&combined) {
            Ok(spec) => {
                tracing::info!("upstream chain: {}", eggproxy_uri::RedactedUri::new(&spec));
                Some(spec)
            }
            Err(e) => {
    ...
    for uri in &listener_uris {
        ...
        let protocols: Vec<eggproxy_core::ProtocolId> = first_hop
            .protocols
            .iter()
            .map(|p| match p {
                eggproxy_uri::ProtocolSpec::Http => "http",
                eggproxy_uri::ProtocolSpec::Socks4 => "socks4",
                eggproxy_uri::ProtocolSpec::Socks5 => "socks5",
            })
            .collect();
    ...
    tracing::info!("eggproxy started, {} listener(s)", listener_uris.len());
...
async fn run_listener(
    ...
    protocols: Vec<eggproxy_core::ProtocolId>,
...
async fn handle_connection(
    ...
    let (target, client_stream) = match proto {
        "http" => {
            let (request, stream) =
                eggproxy_protocol_http::connect::server::handle_connect(client_stream, false, None)
                    .await?;
            (request.target, stream)
        }
        "socks5" => {
            use eggproxy_protocol_socks::socks5::server::{
            ...
            let bind_addr =
                eggproxy_protocol_socks::socks5::server::SocksAddr::IPv4([0, 0, 0, 0], 0);
        ...
        "socks4" => {
            use eggproxy_protocol_socks::socks4::server::{
...
impl HopHandler for HttpHopHandler {
    fn protocol(&self) -> eggproxy_uri::ProtocolSpec {
        eggproxy_uri::ProtocolSpec::Http
    }
    ...
    fn handshake<'a>(
        ...
        Box::pin(async move {
            eggproxy_protocol_http::http_connect(stream, target, auth)
                .await
                .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)
        })
    }
...
impl HopHandler for Socks5HopHandler {
    fn protocol(&self) -> eggproxy_uri::ProtocolSpec {
        eggproxy_uri::ProtocolSpec::Socks5
    }
    ...
    fn handshake<'a>(
        ...
        let auth = credentials.map(|c| (c.username.as_str(), c.password.as_str()));
        Box::pin(async move {
            eggproxy_protocol_socks::socks5::client::socks5_connect(stream, &socks_addr, auth)
                .await
                .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)
        })
    }
...
impl HopHandler for Socks4HopHandler {
    fn protocol(&self) -> eggproxy_uri::ProtocolSpec {
        eggproxy_uri::ProtocolSpec::Socks4
    }
    ...
    fn handshake<'a>(
        ...
        Box::pin(async move {
            eggproxy_protocol_socks::socks4_connect(stream, target, user_id)
                .await
                .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)
        })
    }
...
fn target_to_socks_addr(target: &TargetAddr) -> eggproxy_protocol_socks::socks5::server::SocksAddr {
    use eggproxy_protocol_socks::socks5::server::SocksAddr;
    match &target.host {
...
fn socks_addr_to_target(addr: &eggproxy_protocol_socks::socks5::server::SocksAddr) -> TargetAddr {
    use eggproxy_protocol_socks::socks5::server::SocksAddr;
    match addr {
...
mod tests {
    ...
    #[test]
    fn test_cli_help() {
        assert_cmd::Command::cargo_bin("eggproxy")
            .unwrap()
            .arg("--help")
            .assert()
            .success()
            .stdout(predicates::str::contains("eggproxy"))
    ...
    async fn test_http_proxy_end_to_end() {
        let (echo_addr, echo_jh) = eggproxy_testkit::start_echo_server().await;
