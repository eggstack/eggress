...
async fn test_pproxy_http_server_eggress_client() {
    ...
    // Note: we do NOT pre-open an unused TCP connection to the pproxy
    // server. An idle connection left open to an HTTP proxy can interfere
    // with pproxy's connection handling and cause the relayed connection
    // to receive no data. The wait_for_port check above already confirms
    // reachability.
    ...
    eprintln!("DEBUG http: echo_addr={}", echo_addr);
    eprintln!("DEBUG http: pproxy_port={}", pproxy_port);
    ...
    eprintln!("DEBUG http: chain execution succeeded, writing data");
    conn.write_all(b"pproxy http test").await.unwrap();
    eprintln!("DEBUG http: write complete, flushing");
    conn.flush().await.unwrap();
    eprintln!("DEBUG http: flush complete, reading with timeout");

    // Use timeout-based read instead of shutdown-then-read_to_end.
    // pproxy does not reliably handle half-close (write shutdown);
    // closing the write side can cause pproxy to tear down the entire
    // connection before relaying the echo response. The differential
    // tests use the same timeout-based pattern.
    let mut buf = Vec::new();
    let read_result = tokio::time::timeout(Duration::from_secs(3), conn.read_to_end(&mut buf))
        .await;
    eprintln!("DEBUG http: read result={:?}, buf={:?}", read_result, buf);
...
async fn test_pproxy_socks5_server_eggress_client() {
    ...
    // Connect through pproxy using eggress's chain executor.
    // Note: we do NOT pre-open an unused TCP connection to the pproxy
    // server. An idle connection left open to a proxy can interfere
    // with pproxy's connection handling and cause the relayed connection
    // to receive no data. The wait_for_port check above already confirms
    // reachability.
    let executor = build_executor();
    ...
    // tests use the same timeout-based pattern.
    let mut buf = Vec::new();
    let _ = tokio::time::timeout(Duration::from_secs(3), conn.read_to_end(&mut buf))
        .await;
