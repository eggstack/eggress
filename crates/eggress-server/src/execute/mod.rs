...
use eggress_outbound::target_to_socks_addr;
use eggress_routing::{RouteRequest, SelectedRoute};
use tokio::io::{AsyncReadExt, AsyncWriteExt};

/// Concrete outbound chain-executor factory (single authority in
/// `eggress-outbound`; re-exported here so listener sessions and
/// existing `eggress_server::build_chain_executor` callers share one
/// hop registry and TLS composition).
pub use eggress_outbound::build_chain_executor;
